...
- Tour Stats is a website data view below the Sheet Key, never part of the laminated Song List. It defaults to most-played songs and supports show highlighting, type filtering, and sorting by song, plays, Rarity, Last / usual gap, and last play.
- Rarity is the one playful stat. It uses a collectible-card style ladder: Common, Uncommon, Rare, Double Rare, Ultra Rare, and Hyper Rare. It is backed mainly by plays in the last 100 shows, with lifetime scarcity as a small tie-breaker. The visible supporting text gives actual last-100 and lifetime play counts; no abstract score is shown.
- Rarity symbols follow trading-card rarity language: black circle (Common), black diamond (Uncommon), one black star (Rare), two black stars (Double Rare), two silver stars (Ultra Rare), and three gold stars in a cluster (Hyper Rare). A song new this tour uses a single outlined star. Symbols are inline SVG matched to the badge text height. Silver `#a6a5a1` and gold `#d4a017` are reserved for these symbols and appear nowhere else on the site.
- Last / usual gap compares the current gap with an expected gap blended from current-tour and last-100 cadence. It must stay literal: show the current number of shows since last play and the usual gap. Do not present it as probability, prediction, odds, or "rotation heat."
...
- Mobile Tour Dates use an intrinsic-width date column plus a flexible content column; status sits on its own line below the venue so neither dates nor long locations can compete with it.
- No horizontal page overflow is acceptable at 320px or wider.

## Color

- Site canvas: `#f7f7f6`
- Laminated paper: `#ffffff`
- Ink: `#111111`
- Muted copy: `#5f5a55`
- Marker red: `#d4514f`
- Marker green: `#2d7c52`
